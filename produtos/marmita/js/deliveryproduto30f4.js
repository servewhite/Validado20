// BLOQUEIO DE INTERFERÊNCIA DE SCRIPTS EXTERNOS
// Garante que os cliques nos botões de adicionais funcionem normalmente

// BLOQUEIO CONTRA INTERFERÊNCIA DE SCRIPTS DE RASTREAMENTO
// Garante que os cliques nos botões de adicionais funcionem normalmente
window.addEventListener('click', function(e) {
	if (e.target.classList.contains('adicionarQtdeOpcao') || 
	    e.target.classList.contains('removerQtdeOpcao') ||
	    e.target.parentElement.classList.contains('adicionarQtdeOpcao') ||
	    e.target.parentElement.classList.contains('removerQtdeOpcao')) {
		// Garante que o evento sempre propague corretamente
		console.log('🔒 Bloqueio de interferência ativado para botão de quantidade');
		// O handler jQuery será disparado normalmente
	}
}, false);
...
$(document).ready(function () {
	...
	$(document).on('click', '.btnSair', function (e) {
		...
		$.ajax({
			...
			success: function (data) {
				...
				console.log('Redirecionamento bloqueado (sair do carrinho)');
			},
			error: function () { }
		});
	});
	...
	$(document).on('click', 'button#btFinalizar', function (e) {
		...
		verificarConexaoInternet().then(status => {
			if (status === 'conectado') {
				let urlLoja = $('body').data('urlloja');
				// BLOQUEADO - window.location.href = "loja/" + urlLoja + "/finalizar";
				// Ao invés disso, abrir o modal de dados do cliente
				$('#formDadosCliente').css('display', 'flex').show();
				console.log('Modal de dados aberto - Clique em Finalizar');
			} else {
						...
						if (obj.semEstoque) {
							...
							if (obj.rastreamento) {
								if (obj.pedido) {
									...
									// window.location.href = obj.rastreamento  // BLOQUEADO - Usar modal de dados ao invés
								}
								/*if (obj.pedido) {
									window.open(
										"https://wa.me/55" + obj.telefone + "?text=Oi, meu nome é *" + nome + "* e acabei de fazer o pedido de número *" + obj.pedido + "* no seu cardápio digital.%20%0A%0A%C2%B0%C2%B0%C2%B0%20*Acompanhe seu pedido aqui*%20%C2%B0%C2%B0%C2%B0%0A%0A " + (obj.rastreamento).replace('https://', ''),
								...
								if (obj.telefone) {
									...
									// window.location.href = obj.obrigado  // BLOQUEADO - Usar modal de dados ao invés
								}
							}
						}
						$('#modalCarregando').hide();
	...
	$(document).on('click', '.adicionarQtdeOpcao', function (e) {
		...
		e.stopPropagation();
		e.stopImmediatePropagation();
		console.log('✓ CLIQUE EM ADICIONAR DETECTADO!');
	...
	$(document).on('click', '.removerQtdeOpcao', function (e) {
		...
		e.stopPropagation();
		e.stopImmediatePropagation();
		console.log('✓ CLIQUE EM REMOVER DETECTADO!');
		let minimo = $(this).parents('.tipo').data('minimo');
		let maximo = $(this).parents('.tipo').data('maximo');
...
// Corrigir exibição do preço ao adicionar adicionais
function totalProduto() {
    ...
    $("#detalhesProduto .info2 .tipo").each(function () {
        let opcoes = $(this).find(".opcoes");
        $(opcoes).each(function () {
            let preco = parseFloat($(this).find(".preco span").text().replace(',', '.')) || 0;
            let quantidade = parseInt($(this).find("input.qtdeOpcao").val()) || 0;
            totalAdicionais += preco * quantidade;
        });
    });

    totalProduto += totalAdicionais;
