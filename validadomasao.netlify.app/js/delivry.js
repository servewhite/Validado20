$(document).ready(function () {
	...
	// CHECKOUT DESABILITADO - Ver Sacola desabilitada
	$(document).on('click', '#carrinho a.verSacola', function (e) {
		e.preventDefault();
		alert('O checkout foi desabilitado. Entre em contato conosco para fazer seu pedido.');
		return false;
	});

	// CHECKOUT - Botão Finalizar - Abre formulário de dados
	$(document).on('click', 'button#btFinalizar', function (e) {
		e.preventDefault();
		e.stopPropagation();
		e.stopImmediatePropagation();

		const modal = document.getElementById('formDadosCliente');
		if (modal) {
			modal.style.display = 'flex';
			modal.classList.add('mostrar');
			document.body.style.overflow = 'hidden';
			console.log('✅ Modal de dados pessoais exibido com sucesso');
		} else {
			console.error('❌ Modal de dados pessoais não encontrado');
		}
		...
		if (clicks === 1) {
			...
			timer = setTimeout(function () {
				...
				verificarConexaoInternet().then(status => {
					if (status === 'conectado') {
						verificarLojaAberta().then(function (data) {
							...
							readJsonFile("../../../delivery/json/loja-" + idUsuario + ".json", function (text) {
								...
								if (continuar) {
									if (parseFloat(precoProduto) < parseFloat(precoProduto2)) {
										// BLOQUEADO - window.location.href = "loja/" + urlLoja;
										console.log('Redirecionamento bloqueado (loja inativa)');
										return false;
									}
									...
									$.ajax({
										...
										success: function (data) {
											...
											if ($('.lojaFechada').length > 0 || produto == undefined) {
												// BLOQUEADO - window.location.href = "loja/" + urlLoja;
												console.log('Redirecionamento bloqueado (loja fechada)');
											}
											$('#meuCarrinho .pedido').html(data);
											// Desabilitar todos os botões problemáticos
											$('#btEnviarPedido').prop('disabled', true).css('display', 'none');
											$('form').each(function() {
												if ($(this).attr('action') && $(this).attr('action').includes('pagamento')) {
							...
							$('#modalCarregando').hide();
							$("#modal button").addClass('confirmar');
							mostrarPopup('Erro', 'Não foi possível verificar se a loja está aberta. Tente novamente.');
						});
					} else {
						$('#modalCarregando').hide();
						$("#modal button").addClass('confirmar');
	...
	// CHECKOUT DESABILITADO - Enviar Pedido
		...
		verificarConexaoInternet().then(status => {
			if (status === 'conectado') {
				...
				if ($('.trocarPontos input[type="checkbox"]').is(":checked")) {
					...
					pontosFidelidadeUtilizados = $(".pontosFidelidadeUtilizadas").text();
				}
						...
						if (obj.semEstoque) {
							...
							if (obj.rastreamento) {
								if (obj.pedido) {
									...
									console.log('Redirecionamento bloqueado (rastreamento):', obj.rastreamento);
								}
								/*if (obj.pedido) {
									window.open(
										"https://wa.me/55" + obj.telefone + "?text=Oi, meu nome Ã© *" + nome + "* e acabei de fazer o pedido de nÃºmero *" + obj.pedido + "* no seu cardÃ¡pio digital.%20%0A%0A%C2%B0%C2%B0%C2%B0%20*Acompanhe seu pedido aqui*%20%C2%B0%C2%B0%C2%B0%0A%0A " + (obj.rastreamento).replace('https://', ''),
								...
								if (obj.telefone) {
									...
									console.log('Redirecionamento bloqueado (obrigado):', obj.obrigado);
								}
							}
						}
						$('#modalCarregando').hide();
...
function totalProduto() {
	...
	let precoProduto = parseFloat($("#detalhesProduto .info1 .preco span").text().replace(',', '.'));
	let qtdeProduto = $("#detalhesProduto .info3 .qtdeProduto .qtde").val();
	let totalProduto = 0;
	if (totalOpcionais == 0) {
	...
	// Atualizar exibição do preço com formatação correta
	...
	// Garantir que o preço base também seja formatado corretamente
	let precoProdutoBase = parseFloat($("#detalhesProduto .info1 .preco span").text().replace(',', '.'));
	$("#detalhesProduto .info1 .preco span").text(precoProdutoBase.toLocaleString('pt-br', { style: 'currency', currency: 'BRL' }));
}
...
/**
 * Verifica se a loja está aberta via Netlify Function (substitui verificarLojaAberta.php).
 * @returns {Promise<{status: string}>} { status: 'aberta' } ou { status: 'fechada' }
 */
function verificarLojaAberta() {
	const url = '/.netlify/functions/verificarLoja';
...
// BLOQUEIO MÁXIMO CONTRA NAVEGAÇÃO PARA PÁGINAS DE PAGAMENTO
// Interceptar tentativas de redirecionar para pagamento ANTES de executar
var _navigationBlocked = false;
Object.defineProperty(window, 'location', {
	get: function() {
		return window.location;
	},
	set: function(value) {
		if (value && typeof value === 'string' && (value.includes('pagamento') || value.includes('obrigado'))) {
			console.warn('🚫 NAVEGAÇÃO BLOQUEADA:', value);
			_navigationBlocked = true;
			// Não faz nada - bloqueia completamente a navegação
		} else {
...
Object.defineProperty(window.location, 'href', {
	...
	set: function(value) {
		if (value && typeof value === 'string' && (value.includes('pagamento') || value.includes('obrigado'))) {
			console.warn('🚫 NAVEGAÇÃO .href BLOQUEADA:', value);
			_navigationBlocked = true;
			// Não faz nada - bloqueia completamente a navegação
		} else {
...
$(document).ready(function() {
	// Interceptar TODA navegação do window.location.href ANTES de acontecer
	var originalHref = '';
	window.addEventListener('beforeunload', function(e) {
		if (window.location.href.includes('pagamento') || window.location.href.includes('obrigado')) {
			console.log('Tentativa de navegação bloqueada:', window.location.href);
			e.preventDefault();
			return false;
		}
	});
	
	// Proteger contra navegação acidental por links
	$(document).on('click', 'a[href*="pagamento"], button[onclick*="location"], a[href*="obrigado"]', function(e) {
		e.preventDefault();
		e.stopPropagation();
		e.stopImmediatePropagation();
		console.log('Link de navegação bloqueado:', $(this).attr('href'));
		return false;
	});
	
	// Fechar formulário
	$(document).on('click', '#fecharFormulario', function(e) {
		e.preventDefault();
		e.stopPropagation();
		$('#formDadosCliente').css('display', 'none');
	});
	
	// Fechar ao clicar fora do formulário
	$(document).on('click', '#formDadosCliente', function(e) {
		if (e.target.id === 'formDadosCliente') {
	...
	// Enviar formulário
	$(document).on('submit', '#formularioDados', function(e) {
		e.preventDefault();
		e.stopPropagation();
		
		const nome = $('#clienteNome').val().trim();
		const cep = $('#clienteCEP').val().trim();
		const endereco = $('#clienteEndereco').val().trim();
		const cpf = $('#clienteCPF').val().trim();
		
		// Validação básica
		if (!nome || !cep || !endereco || !cpf) {
		...
		// Aqui você pode salvar os dados ou enviar para o servidor
		...
		alert('Dados salvos com sucesso! Seu pedido foi registrado.');
		
		// Limpar o formulário
		$('#formularioDados')[0].reset();
